...
import type { CompraMonedaExtranjera, CreadoPorRef } from '../../../../models/compra-moneda-extranjera';

@Component({
  selector: 'app-compra-detalle-modal',
  standalone: true,
  imports: [DatePipe, GlassModalComponent, StatusBadgeComponent, ArsCurrencyPipe],
  template: `
    <app-glass-modal
      [open]="open()"
      title="Detalle de Compra FX"
          ...
          <div class="section">
            ...
            <div class="info-grid">
              <div class="info-row">
                ...
                <span class="info-value num">USD {{ fmtUSD(compra()!.montoUSD) }}</span>
              </div>
              @if (compra()!.tipoCambio != null) {
                <div class="info-row">
                  <span class="info-label">Tipo de Cambio</span>
                  <span class="info-value num">{{ fmtUSD(compra()!.tipoCambio!) }}</span>
                </div>
              }
              @if (compra()!.montoARS != null) {
                <div class="info-row">
                  <span class="info-label">Monto ARS</span>
                  <span class="info-value num">{{ compra()!.montoARS! | arsCurrency }}</span>
                </div>
              }
              @if (compra()!.comision > 0) {
                <div class="info-row">
                  <span class="info-label">Honorarios</span>
                  <span class="info-value num">{{ compra()!.comision | arsCurrency }}</span>
                </div>
              }
            </div>
          </div>
