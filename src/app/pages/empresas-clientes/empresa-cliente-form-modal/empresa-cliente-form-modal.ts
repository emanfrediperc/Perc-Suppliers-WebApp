...
@Component({
  selector: 'app-empresa-cliente-form-modal',
  standalone: true,
  imports: [FormsModule, GlassModalComponent],
  template: `
    <app-glass-modal [open]="open()" [title]="isEdit() ? 'Editar Empresa Cliente' : 'Nueva Empresa Cliente'" (close)="onClose()">
...
export class EmpresaClienteFormModalComponent {
  open = input(false);
  entity = input<EmpresaCliente | null>(null);
  close = output<void>();
  saved = output<void>();
  saving = signal(false);
